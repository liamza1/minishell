...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/07/20 07:37:25 by lstreak           #+#    #+#             */
/*   Updated: 2017/09/20 15:07:05 by lstreak          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			is_exec(char *bin_path, struct stat f, char **command)
{
...
static int			check_bin(char **command)
{
	int				i;
	char			*bin_path;
	char			**path;
	...
	path = ft_strsplit(get_envv("PATH"), ':');
	i = -1;
	while (path && path[++i])
	{
...
int					exec_comm(char **command)
{
	...
	if ((is_builtin = check_builtins(command)) == 1 || check_bin(command))
		return (0);
	if (is_builtin < 0)
		return (-1);
